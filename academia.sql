...
-- Visão 1: Relatório simplificado de alunos ativos e seus planos
CREATE OR REPLACE VIEW vw_alunos_ativos AS
SELECT 
    p.id_pessoa AS id_aluno,
...
JOIN Plano pl ON m.id_plano = pl.id_plano
WHERE m.status = 'ativa';

-- Visão 2: Ficha de Treino detalhada (Traduzida com nomes)
...
SELECT 
    ...
    it.tempo_descanso
FROM Treino t
JOIN Pessoa pa ON t.id_aluno = pa.id_pessoa
JOIN Pessoa pp ON t.id_professor = pp.id_pessoa
JOIN Item_Treino it ON t.id_treino = it.id_treino
JOIN Exercicio e ON it.id_exercicio = e.id_exercicio;


-- Trigger 1: Calcular automaticamente a data_fim da matrícula baseada na duração do plano
CREATE TRIGGER trg_calcula_data_fim_matricula
BEFORE INSERT ON Matricula
FOR EACH ROW
BEGIN
    DECLARE meses_duracao INT;
    
    -- Busca a duração do plano em meses
    SELECT duracao INTO meses_duracao 
    FROM Plano 
    WHERE id_plano = NEW.id_plano;
    
    -- Se a data_fim for enviada como NULL, calcula automaticamente
    IF NEW.data_fim IS NULL THEN
        SET NEW.data_fim = DATE_ADD(NEW.data_inicio, INTERVAL meses_duracao MONTH);
    END IF;
END; //

-- Trigger 2: Alterar o status do Aluno caso sua matrícula mude de situação
CREATE TRIGGER trg_atualiza_status_aluno
AFTER UPDATE ON Matricula
FOR EACH ROW
BEGIN
    -- Se a matrícula deixar de ser ativa, inativa o aluno
    IF NEW.status != 'ativa' AND OLD.status = 'ativa' THEN
        UPDATE Aluno 
        SET status = 'inativo' 
        WHERE id_pessoa = NEW.id_aluno;
    -- Se uma matrícula voltar a ficar ativa, ativa o aluno novamente
    ELSEIF NEW.status = 'ativa' AND OLD.status != 'ativa' THEN
        UPDATE Aluno 
        SET status = 'ativo' 
        WHERE id_pessoa = NEW.id_aluno;
    END IF;
